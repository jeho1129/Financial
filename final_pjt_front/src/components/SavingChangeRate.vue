    ...
    <div class="position-relative p-4 heightFull">
      <form method="dialog" class="position-absolute move">
        <button value="close" id="closeSavingChangeRateDialog">
          <font-awesome-icon :icon="['fas', 'xmark']" class="closeSavingChangeRateDialog" />
        </button>
      </form>
      <h2 class="text-center">금리 정보 변경</h2>
      <form @submit.prevent="changeRate" class="d-flex flex-column" id="savingChangeRateForm">
        <div v-for="(option, index) in savingData.savingoptions_set" :key="option.id" class="d-flex flex-column gap-2">
          <h4 class="m-0">{{ option.save_trm }}개월 ({{ option.intr_rate_type_nm }})</h4>
          <div>
            <label for="normalRate">기본 금리 </label>
            <br />
            <input v-model.trim="savingData.savingoptions_set[index].intr_rate" type="number" style="width: 100%" step="0.01" id="save_trm" required />
          </div>
          <div>
            <label for="bestRate">우대 금리 </label>
            <br />
            <input v-model.trim="savingData.savingoptions_set[index].intr_rate2" type="number" style="width: 100%" step="0.01" id="bestRate" required />
          </div>
          <hr />
        </div>
...
const changeRate = () => {
  if (window.confirm("금리 정보를 변경 하시겠습니까?")) {
    ...
    axios({
      ...
      .then((res) => {
        const modal = document.querySelector("#moveSavingChangeRate");
        modal.close();
      })
      .catch((err) => {
        console.log(err);
