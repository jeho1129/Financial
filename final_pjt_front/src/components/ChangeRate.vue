...
const changeRate = () => {
  if (window.confirm("금리 정보를 변경 하시겠습니까?")) {
    const data = props.depositData.depositoptions_set.map((item) => {
      return {
        id: item.id,
    ...
    axios({
      method: "put",
      url: `${authStore.API_URL}/banking/deposits/change/${props.depositData.fin_prdt_cd}/ `,
      headers: {
        Authorization: `Token ${authStore.token}`,
      },
      data: JSON.stringify({
        depositoptions_set: data,
      }),
    })
      .then((res) => {
