...
- 금리 비교 차트
  ...
  - 회원이 가입한 금융상품들의 기존 금리와 우대 금리를 비교하여 사용자에게 시각적으로 볼 수 있게 구현하였습니다.

- 관리자 게시판, 댓글 삭제

  - 관리자가 모든 회원의 게시판과 댓글을 삭제할 수 있게 구현하였습니다.

- 관리자 금융 상품 금리 수정
  - UI는 만들었지만 아직 기능이 동작하지 않아 내일 다시 시도해야 합니다.

## 설계 내용 및 데이터베이스 모델링(ERD)

- Component 구조도
  ![Component](final_pjt_back/Component.png)
- Django URL 정리
...
## 서비스 대표 기능
1. 메인 페이지
  - 우선 상단 NavBar를 이용하여 해당 기능으로 이동할 수 있는 기능을 구현하였습니다.
  - 상단 회원가입, 로그인, 로그아웃 버튼을 구현하여 간편하게 인증을 할 수 있도록 하였습니다.
  - 메인 페이지에 인기 예금, 적금 상품을 나열하여 소비자가 쉽게 금융 상품 트렌드를 파악할 수 있도록 하였습니다.
2. 회원 커스터마이징
  - Django-rest-auth 라이브러리를 이용하여 Django 내에서 커스텀 User를 구현하였습니다. 기본 입력 정보인 username과 email, password뿐만이 아니라, 이름, 직업, 연봉, 자산을 추가적으로 입력할 수 있도록 설정하였고, user가 금융 상품에 가입할 때, 해당 금융 상품의 정보와 옵션까지 확인할 수 있도록 JSONField를 이용하여 금융 상품 정보를 담을 수 있도록 하였습니다.
3. 예적금 금리 비교
  - 데이터 저장 : 금융상품통합비교공시 API를 이용하여 정기 예금, 적금 상품 데이터를 가져와 DB에 저장하였습니다. 또한 filter를 활용하여 이미 존재하는 상품일 경우, DB에 저장되지 않도록 추가적으로 설정하였습니다.
  - 전체 조회 : Vue.js를 통해 상품 전체 목록을 볼 수 있도록 화면을 구성하였습니다. 은행 및 금융 옵션을 선택하여 소비자가 원하는 옵션을 필터링할 수 있도록 화면을 구성하였습니다.
  - 상세 조회 : 특정 상품 클릭 시 해당 상품에 대한 상세한 정보를 볼 수 있도록 하였고, 상품 내 옵션을 출력하여 해당 옵션을 선택하여 금액 입력 시 예상 이자 금액을 출력할 수 있도록 설정하였습니다. 또한 가입 기능을 통해 가입하기 버튼을 클릭 시 해당 회원의 금융 상품 목록 필드에 저장할 수 있도록 하였습니다. 가입한 사람들에게는 가입취소 버튼이 떠 가입을 취소할 수도 있습니다.
  - 관리자 기능 : 관리자 계정으로 접속 시, 금융 상품의 금리 정보를 수정할 수 있도록 하였습니다. 또한 Django-core-mail 모듈을 통해 상품 금리 정보가 수정될 시 해당 상품에 가입한 유저의 이메일로 변경 금리 정보가 담긴 메일이 전송되도록 하였습니다.
4. 환율 계산기
  - 한국수출입은행 환율정보 API를 이용하여 백엔드에서 데이터를 받아와 프론트로 바로 전달할 수 있도록 설정하였습니다.
  - Vue.js를 통해 국가를 선택하고, 원화나 타국 통화 입력 시 해당 입력 값에 맞게 변환된 값을 출력하여 확인할 수 있도록 하였습니다.
5. 근처 은행 검색
  - KaKao Maps API를 활용하여 지도 데이터를 가져왔습니다. 또한 원하는 위치를 선택할 시 해당 위치 근처의 은행 정보를 출력할 수 있도록 설정하였습니다.
6. 커뮤니티(게시판)
  - 회원 간 정보 공유를 위한 커뮤니티 기능을 구현하기 위해 글, 댓글 Model을 생성하였습니다.
  - 회원이 아닐 경우, 조회 기능만, 회원일 경우 게시글 및 댓글 생성, 해당 회원이 작성한 게시글, 댓글 수정 / 삭제 기능이 가능하도록, 또한 관리자 계정일 경우에는 모든 회원의 게시글과 댓글을 삭제할 수 있도록 설정하였습니다.
7. 프로필 페이지
  - 해당 회원의 모든 정보를 표시하고, 정보를 수정할 수 있도록 프로필 페이지를 구성하였습니다.
  - 또한 해당 회원이 가입한 예, 적금 상품 리스트를 표시하고, chartjs 라이브러리를 활용하여 해당 회원이 가입한 상품 금리 정보를 그래프로 출력할 수 있도록 하였습니다.
8. 금융 상품 추천 알고리즘
  - 해당 회원의 프로필 페이지에서 회원 정보에 맞게 금융 상품을 추천하는 화면도 추가적으로 구현하였습니다.
...
- 정태윤
  - 이번 프로젝트에서 가장 인상 깊었던 경험 중 하나는 백엔드에서 제공한 데이터를 프론트엔드에서 Vue.js를 이용해 사용자에게 직관적으로 보여주는 과정이었습니다. Vue.js의 다양한 기능을 활용하여 백엔드에서 제공하는 RESTful API를 효율적으로 연결하고, 이를 통해 데이터를 웹 페이지에 성공적으로 출력한 모습은 매우 인상적이었습니다.
  - Vue.js의 컴포넌트 기반 구조를 이용해 페이지를 모듈화하고, 이를 통해 코드의 재사용성을 높이고 유지 관리를 편리하게 한 점은 큰 장점이었습니다. 또한, 동적 바인딩 기능을 통해 사용자의 상호작용에 따라 웹 페이지의 내용이 실시간으로 업데이트되는 점도 사용자 경험을 크게 향상시켰습니다.
  - 이러한 과정을 통해 백엔드와 프론트엔드가 원활하게 연동되어야 웹 서비스가 제대로 동작한다는 점을 다시 한번 깨달았습니다. 실제로 Vue.js를 통해 데이터를 사용자에게 효과적으로 전달함으로써, 사용자 친화적이고 직관적인 인터페이스를 제공할 수 있게 되었습니다.
  - 이러한 경험은 프론트엔드와 백엔드가 서로를 보완하며 웹 서비스를 구성한다는 점을 확실하게 인지하게 해주었고, 이를 통해 팀워크의 중요성을 다시 한번 깨닫게 해주었습니다. 앞으로 이런 경험을 바탕으로 더욱 효과적인 웹 서비스를 개발하는 데 기여하겠습니다.